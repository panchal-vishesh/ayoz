...
const SOCIALS = [
  {
    label: 'Instagram',
    href: 'https://www.instagram.com/ayoz.in',
    icon: () => (
  ...
  {
    label: 'LinkedIn',
    href: 'https://linkedin.com/company/ayoz-in',
    icon: () => (
      <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8">
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="max-w-sm">
            <div className="flex items-center gap-3">
              <div className="rounded-[14px] border border-[#1F2A3D] bg-[#101726] p-2">
                <img
          ...
          <div className="flex flex-col gap-2">
            ...
            <a href="mailto:hello@ayoz.in" className="text-sm text-slate-400 transition-colors hover:text-brand-soft">
              hello@ayoz.in
            </a>
            <a href="mailto:support@ayoz.in" className="text-sm text-slate-400 transition-colors hover:text-brand-soft">
              support@ayoz.in
            </a>
            <a href="mailto:partners@ayoz.in" className="text-sm text-slate-400 transition-colors hover:text-brand-soft">
              partners@ayoz.in
            </a>
          </div>

          {/* Status card */}
          <div className="rounded-2xl border border-[#1F2A3D] bg-[#151D2E] p-4 sm:p-5 lg:w-64">
            <div className="flex items-center justify-between">
              <p className="text-[0.62rem] font-semibold uppercase tracking-[0.22em] text-slate-400">
                Platform status
              </p>
              <div className="flex items-center gap-1.5 rounded-full border border-[#FF6B1A]/20 bg-[#FF6B1A]/[0.07] px-2.5 py-1">
                <motion.span
                  className="h-1.5 w-1.5 rounded-full bg-[#FFB347]"
                  animate={{ scale: [1, 1.5, 1], opacity: [1, 0.5, 1] }}
                  transition={{ duration: 2, repeat: Infinity }}
                  style={{ boxShadow: '0 0 8px rgba(255,179,71,0.9)' }}
                />
                <span className="text-[0.58rem] font-semibold uppercase tracking-[0.18em] text-[#FFB347]/80">
                  Live
                </span>
              </div>
            </div>
        ...
        <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {FEATURES.map(({ icon: Icon, title, text }) => (
            <div key={title} className="rounded-2xl border border-[#1F2A3D] bg-[#151D2E] p-4">
